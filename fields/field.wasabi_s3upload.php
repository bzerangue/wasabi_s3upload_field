...
require_once(EXTENSIONS .'/wasabi_s3upload_field/lib/class.s3Facade.php');

class FieldWasabi_S3Upload extends FieldUpload
{
    ...
    {
        parent::__construct();
        $this->_name = 'Wasabi S3 Upload';
        $this->_driver = Symphony::ExtensionManager()->create('wasabi_s3upload_field');

        $this->s3 = new S3FacadeWasabi(
    ...
    private function getUrl($file)
    {
        ...
        $bucketRegion = $this->s3->getBucketLocation($this->get('bucket'));
        $bucketRegionUrl = "s3." . $bucketRegion . ".wasabisys.com";

        if ($this->get('cname') == '') {
            $url = $protocol . $bucketRegionUrl . $this->get('bucket') . "/" . $file;
        }
        else {
    ...
    private function getUniqueFilename(&$file)
    {
        ...
        $crop  = '30';
        $file = preg_replace_callback("/(.*)(\.[^\.]+)/", function ($matches) use ($crop) {
    ...
    public function commit()
    {
        ...
        $fields['validator'] = ($fields['validator'] ?? null == 'custom' ? NULL : $this->get('validator'));
    ...
    public function displayPublishPanel(XMLElement &$wrapper, $data = null, $flagWithError = null, $fieldnamePrefix = null, $fieldnamePostfix = null, $entry_id = null)
    {
        ...
        if($data['file'] ?? null) $span->appendChild(Widget::Anchor($this->getUrl($data['file'] ?? null), $this->getUrl($data['file'] ?? null)));

        $span->appendChild(Widget::Input('fields'.$fieldnamePrefix.'['.$this->get('element_name').']'.$fieldnamePostfix, $data['file'] ?? null, ($data['file'] ?? null ? 'hidden' : 'file')));
    ...
    {
        if(!$file = $data['file'] ?? null) return NULL;
